...
1. **malagu build：** 构建应用，一般情况不使用，部署应用的时候自动使用该命令构建项目
...
1. **malagu info：** 查询部署在云端的函数和触发器信息。该命令是一个上下文命令，只要满足一定上下文要求才会看到该命令，所有需要在项目根目录执行该命令，并且项目依赖了 FaaS 适配器相关的组件，例如 `fc-adapter` 、 `scf-adapter` 等等。
