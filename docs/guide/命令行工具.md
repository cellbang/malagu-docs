...
注意：一般情况，不需要手动执行构建命令，因为在执行部署命令的时候会自动构建，部署命令可以通过选项 `-s, --skip-build` 跳过构建过程。如果你不打算使用框架提供的部署命令，此时，你需要自己手动执行构建命令。不过，当框架提供的部署命令不满足你的业务需要，更推荐的做法是通过提供一个命令 `[Deploy](#0ji38)`[ 插件](#0ji38)，自定义部署逻辑。
