---
title: 创建项目
description: 本篇通过使用Malagu框架编写Blog来演示相关组件用法
type: learn
lang: zh-CN
---

### 创建项目

#### 创建目录及初始化

```bash
mkdir malagu-blog
cd malagu-blog
echo '{}' > package.json
npm i -D @malagu/cli @malagu/cli-service
npm i -S @malagu/core @malagu/mvc @malagu/typeorm
```

#### 编辑package.json

加入以下内容:

```json
{
    "name": "vue-example",
    "keywords": ["malagu-component"],
    "scripts": {
        "start": "malagu serve",
        "build": "malagu build"
    },
}
```
* keywords必须添加且其中必须有`malagu-component`，框架通过此配置来循环查找依赖链

#### 添加文件

tsconfig.json
```json
{
    "compilerOptions": {
        "target": "esnext",
        "module": "commonjs",
        "strict": true,
        "jsx": "preserve",
        "moduleResolution": "node",
        "skipLibCheck": true,
        "declaration": true,
        "declarationMap": true,
        "noImplicitAny": true,
        "noEmitOnError": false,
        "noImplicitThis": true,
        "noUnusedLocals": true,
        "strictNullChecks": true,
        "experimentalDecorators": true,
        "emitDecoratorMetadata": true,
        "downlevelIteration": true,
        "strictPropertyInitialization": false,
        "lib": [
            "es6",
            "dom"
        ],
        "sourceMap": true,
        "rootDir": "src",
        "outDir": "lib",
        "baseUrl": "src",
        "paths": {
            "~/*": ["*"]
        }
    },
    "include": [
        "src"
    ]
}
```

* 添加ts配置

malagu.yml

```yaml
backend:
  modules:
    - src/backend/module
```

* 添加Malagu项目配置

src/backend/controllers/home-controller.ts

```ts
import { Controller, Get, Text } from "@malagu/mvc/lib/node";

@Controller('api/home')
export class HomeController {
    @Get()
    @Text()
    index(): string {
        return "hello";
    }
}
```

src/backend/controllers/index.ts

```ts
export * from "./home-controller";
```

src/backend/module.ts

```ts
import { autoBind } from "@malagu/core";
import "./controllers";

export default autoBind();
```

本地运行`npm start`，用curl测试接口：`curl http://localhost:3000/api/home`此时应该能看到接口输出

### 使用migration创建数据表

#### 初始化typeorm

```bash
npx typeorm init
```
清理typeorm自动创建但未使用到的文件
- src/index.ts
- src/entity

编辑ormconfig.json，修改数据库链接信息


#### 用migration创建表

执行以下命令创建migration

```bash
npx typeorm migration:create -n posts
npx typeorm migration:create -n categories
npx typeorm migration:create -n tags
npx typeorm migration:create -n pots-tag
```

生成的脚本文件在src/migration/目录，修改src/migration/xxxx-posts.ts

```ts
import {MigrationInterface, QueryRunner, Table, TableIndex} from "typeorm";

export class posts1632587486617 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.createTable(new Table({
            name: "posts",
            columns: [
                { name: "id", type: "int", isPrimary: true },
                { name: "category_id", type: "int", isNullable: false },
                { name: "title", type: "varchar", isNullable: false },
                { name: "content", type: "longtext", },
                { name: "desc", type: "varchar" },
                { name: "created_at", type: "datetime", isNullable: false },
                { name: "updated_at", type: "datetime", isNullable: false }
            ]
        }), true);
        await queryRunner.createIndex("posts", new TableIndex({
            name: "posts_category_id_index",
            columnNames: ["category_id"]
        }));
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.dropTable("posts")
    }

}
```

修改src/migration/xxxx-categories.ts

```ts
import {MigrationInterface, QueryRunner, Table} from "typeorm";

export class categories1632671182234 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.createTable(new Table({
            name: "categories",
            columns: [
                { name: "id", type: "int", isPrimary: true  },
                { name: "parent_id", type: "int", isNullable: false },
                { name: "title", type: "varchar", isNullable: false },
                { name: "level", type: "int", isNullable: false },
                { name: "desc", type: "varchar" },
                { name: "created_at", type: "datetime", isNullable: false },
                { name: "updated_at", type: "datetime", isNullable: false }
            ]
        }), false);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.dropTable("categories");
    }

}
```

修改src/migration/xxxx-tags.ts

```ts
import {MigrationInterface, QueryRunner, Table} from "typeorm";

export class tags1632672024877 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.createTable(new Table({
            name: "tags",
            columns: [
                { name: "id", type: "int", isPrimary: true  },
                { name: "title", type: "varchar", isNullable: false },
                { name: "desc", type: "varchar" },
                { name: "created_at", type: "datetime", isNullable: false },
                { name: "updated_at", type: "datetime", isNullable: false }
            ]
        }), false);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.dropTable("tags");
    }
}
```

修改src/migration/xxxx-post-tag.ts

```ts
import {MigrationInterface, QueryRunner, Table} from "typeorm";

export class postTag1632672762069 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.createTable(new Table({
            name: "post_tag",
            columns: [
                { name: "post_id", type: "int", isNullable: false },
                { name: "tag_id", type: "int", isNullable: false }
            ]
        }), false);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.dropTable("post_tag");
    }

}
```

#### 执行migration脚本

```bash
npx ts-node --transpile-only ./node_modules/typeorm/cli.js migration:run
```
